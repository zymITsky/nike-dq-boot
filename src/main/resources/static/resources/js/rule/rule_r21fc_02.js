...
var R21fcViewModel = function () {
		...
		if (!newValue) {
			if (!self.readyForFocused) {
				...
				if (self.txtRuleCaseName() != null && self.txtRuleCaseName() != '') {
					$.ajax({
						url:  _contextRoot + '/rule/r21fc/new/form/api/is_name_duplicated/' + self.txtRuleCaseName(),
						type: 'GET',
	...
	self.submit = function () {
		if (self.errors().length == 0) {
			$.ajax({
				url:  _contextRoot + '/rule/r21fc/new/form/api/new_case',
				type: 'POST',
				data: ko.toJSON(self),
				contentType: 'application/json;charset=utf-8',
				success: function (data) {
					if (data.status == "Success") {
						window.location.replace(_contextRoot + "/rule/r21fc/list/form");
					} else {
						$.prompt("Case created failed.<br>Please check details of connection, db, table and field.<br><br>" + data.message);
					}
				},
				error: function (e) {
					alert("error - " + error.status);
...
$(document).ready(function () {
	...
	var vmR21fc = new R21fcViewModel();
	ko.applyBindings(vmR21fc, $('#frmR21fc').get(0));
	vmR21fc.getConnections();
	vmR21fc.getBfs();
});